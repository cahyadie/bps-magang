...
html {
    ...
    overscroll-behavior: none; /* <-- Tambahkan baris ini */
}
        ...
        .card-wrapper:nth-child(1) .magang-card { animation-delay: 0.1s; }
        .card-wrapper:nth-child(2) .magang-card { animation-delay: 0.2s; }
        .card-wrapper:nth-child(3) .magang-card { animation-delay: 0.3s; }
        .card-wrapper:nth-child(4) .magang-card { animation-delay: 0.4s; }
        .card-wrapper:nth-child(5) .magang-card { animation-delay: 0.5s; }
        .card-wrapper:nth-child(6) .magang-card { animation-delay: 0.6s; }
        .card-wrapper:nth-child(n+7) .magang-card { animation-delay: 0.7s; }
        ...
        .view-toggle-btn.active {
            background-color: rgba(217, 119, 87, 0.4); /* Brand color semi-transparent */
            color: #e88968; /* Brand color */
...
<body>
                ...
                <div class="flex items-center gap-3">
                    @auth
                        ...
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit"
                                    class="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white px-5 py-2.5 rounded-xl font-medium transition-all duration-300 inline-flex items-center gap-2 text-sm shadow-lg shadow-red-600/30 hover:shadow-xl hover:shadow-red-600/40 hover:-translate-y-0.5">
                                ...
                                @if(auth()->user()->isAdmin())
                                    <span
                                        ...
                                        <i class="fas fa-crown"
                                            style="font-size: 0.625rem; margin-right: 0.25rem;"></i>ADMIN
                                    </span>
                                @else
                                    <span
                                        ...
                                        <i class="fas fa-user"
                                            style="font-size: 0.625rem; margin-right: 0.25rem;"></i>USER
                                    </span>
                                @endif
                            </div>
                        </div>
    ...
    <div class="max-w-7xl mx-auto py-8">
        @if (session('success'))
            <div class="px-6"> <div class="success-alert">
                <span>{{ session('success') }}</span>
            </div>
            </div>
        @endif

       <!-- ✅ FILTER SECTION (AUTO SUBMIT) -->
<div class="px-6"> 
        ...
        @if (count($magangs) > 0)
            <div id="magang-container" class="flex overflow-x-auto gap-6 py-4 hide-scrollbar px-6">
                @foreach ($magangs as $magang)
                    <div class="card-wrapper flex-shrink-0 w-96"> <div class="magang-card"
                        @if ($magang->foto) 
        ...
        @else
            ...
            <div class="empty-state">
                ...
                @else
                    ...
                    <p class="text-[#6a6a6a] text-sm mt-2">Klik tombol "Tambah Data Magang" untuk mulai menambahkan data</p>
                @endif
            </div>
        @endif
    ...
    const body = document.body;
        ...
        if (resultsContainer) { // Pastikan kontainernya ada
            resultsContainer.addEventListener('wheel', function(e) {
                // Cek jika yang di-scroll adalah mouse wheel vertikal (e.deltaY)
                if (body.classList.contains('view-row') && e.deltaY != 0) {
                    // Hentikan perilaku default (mencegah halaman scroll ke atas/bawah)
                    e.preventDefault();
                    
                    // Pindahkan scroll horizontal kontainer
                    // e.deltaY berisi "jarak" scroll. 
                    // Kita bisa kalikan untuk membuatnya lebih cepat/lambat
                    resultsContainer.scrollLeft += e.deltaY * 1; // Ubah angka '1' jika terlalu lambat/cepat
                }
            });
        }
        ...
        function performFilter() {
            ...
            .then(html => {
                ...
                const newResults = doc.getElementById('magang-container'); // ✅ GANTI KE SINI
