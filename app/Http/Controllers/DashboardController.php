    ...
    public function index(Request $request)
{
    ...
    $role = Auth::user()->role;
    $today = Carbon::now(); // Variable hari ini untuk filter

    // ===========================================
    // 1. BUAT VARIABLE $activeInterns (SOLUSI ERROR)
    // ===========================================
    // Ambil pendaftar yang statusnya approved DAN hari ini masih dalam periode magang
    $activeInterns = Pendaftaran::where('status', 'approved')
    ...
    if ($role === 'admin') {
        ...
        $calendarEvents = $queryKalender->get()->flatMap(function ($pendaftaran) {
            return [
                [
                    ...
                    'start' => $pendaftaran->tanggal_mulai,
                    'display' => 'list-item',
                    'extendedProps' => ['icon' => 'play']
                ],
                [
                    'title' => $pendaftaran->nama_pendaftar . ' (Selesai)',
                    'start' => $pendaftaran->tanggal_selesai,
                    'display' => 'list-item',
        ...
        return view('dashboard', [
            ...
            'activeInterns' => $activeInterns, // <--- TAMBAHKAN INI (PENTING)
        ]);
    ...
    // ===========================================
        ...
        return view('dashboard', [
            ...
            'calendarEvents' => $calendarEvents,
            'activeInterns' => $activeInterns, // <--- TAMBAHKAN INI JUGA DI SINI
        ]);
    }
